...
def options_summary():
    ...
    for exp_date, prices_dict in call_expiration_date_map.items():
        if OptionsSupport.days_to_expiration(exp_date) > 1:
            vol_sorted_contracts = OptionsSupport.volume_sorted_contracts(prices_dict)
            oi_sorted_contracts = OptionsSupport.open_interest_sorted_contracts(prices_dict)
    ...
    return json.dumps({"calls": calls_summary_dict, "puts": puts_summary_dict})
